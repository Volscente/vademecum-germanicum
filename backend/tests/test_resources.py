...
def test_create_resource_success(client, valid_resource_payload):
    """
...
def test_get_resources_list(client, valid_resource_payload):
    """
...
def test_search_resources_success(client, valid_resource_payload):
    """
    Ensure the search filter correctly works based on name and description.
    """
    client.post("/resources/", json=valid_resource_payload)

    response = client.get("/resources/?search=Tagesschau")
    assert response.status_code == 200
    assert len(response.json()) >= 1
    assert response.json()[0]["name"] == "Tagesschau"

    response = client.get("/resources/?search=German news")
    assert response.status_code == 200
    assert len(response.json()) >= 1

    response = client.get("/resources/?search=NonExistentResource")
    assert response.status_code == 200
    assert len(response.json()) == 0


def test_update_resource_success(client, valid_resource_payload):
    """
...
def test_update_resource_not_found(client):
    ...
    """
    response = client.put("/resources/9999", json={})
    assert response.status_code == 404


def test_delete_resource_success(client, valid_resource_payload):
    """
...
def test_create_resource_duplicate_url_returns_409(client, valid_resource_payload):
    """
...
def test_update_resource_duplicate_url_returns_409(
    client, valid_resource_payload, second_resource_payload
):
    """
    Ensure updating a resource's URL to collide with another resource returns HTTP 409.
    """
    client.post("/resources/", json=valid_resource_payload)
    response = client.post("/resources/", json=second_resource_payload)
