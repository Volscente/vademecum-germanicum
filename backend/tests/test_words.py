...
def test_create_word_success(auth_client, valid_word_payload):
    """
...
def test_get_words_list(auth_client):
    """
...
def test_search_words_success(auth_client, valid_word_payload):
    """
    ...
    # Search by Word
    response = auth_client.get("/words/?search=Zuschlag")
    assert response.status_code == 200
    assert len(response.json()) >= 1
    assert response.json()[0]["word"] == "Zuschlag"

    # Search by Translation
    response = auth_client.get("/words/?search=Surcharge")
    assert response.status_code == 200
    assert len(response.json()) >= 1
    assert response.json()[0]["translation"] == "Surcharge"

    # Word not found
    response = auth_client.get("/words/?search=NonEsistentWord")
    assert response.status_code == 200
    assert len(response.json()) == 0


def test_update_word_success(auth_client, valid_word_payload):
    """
    ...
    response = auth_client.put(f"/words/{word_id}", json=update_payload)
...
def test_update_word_not_found(auth_client):
    """
    Ensure the status code 404 is returned.
    """
    response = auth_client.put("/words/9999", json={})
    assert response.status_code == 404


def test_delete_word_success(auth_client, valid_word_payload):
    """
    ...
    # Delete word
    response = auth_client.delete(f"/words/{word_id}")
...
def test_delete_word_not_found(auth_client):
    """
    Ensure the status code 404 is returned.
    """
    response = auth_client.delete("/words/9999")
    assert response.status_code == 404


def test_create_word_with_multiple_senses(auth_client, two_senses_payload):
    """
    Ensure a word created with multiple senses persists all senses with their
    grammar patterns and example sentences.
    """
    response = auth_client.post("/words/", json=two_senses_payload)
    assert response.status_code == 200
    data = response.json()
...
def test_create_word_empty_senses_rejected(auth_client, valid_word_payload):
    """
    Ensure a word with an empty senses list is rejected with HTTP 422.
    """
    payload = {**valid_word_payload, "senses": []}
    response = auth_client.post("/words/", json=payload)
    assert response.status_code == 422


def test_create_word_empty_grammar_patterns_rejected(auth_client, valid_word_payload):
    """
...
def test_create_word_empty_example_sentences_rejected(auth_client, valid_word_payload):
    """
...
def test_update_word_replaces_senses(auth_client, valid_word_payload, two_senses_payload):
    """
...
def test_update_word_without_senses_preserves_existing(auth_client, valid_word_payload):
    """
    ...
    response = auth_client.put(f"/words/{word_id}", json={"translation": "Extra charge"})
    assert response.status_code == 200
    data = response.json()
...
def test_create_word_duplicate_returns_409(auth_client, valid_word_payload):
    """
...
def test_create_word_with_provided_review_state(auth_client, valid_word_payload):
    """
...
def test_create_word_without_review_state_defaults(auth_client, valid_word_payload):
    """
...
def test_create_word_case_sensitive_duplicate(auth_client, valid_word_payload):
    """
    ...
    response_lower = auth_client.post("/words/", json=lower_payload)
    response_upper = auth_client.post("/words/", json=upper_payload)
