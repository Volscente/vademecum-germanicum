...
- `AddWordModal`'s `isDuplicate` state is deliberately kept outside `wordSchema`/Zod: React Hook Form's async resolver only re-validates on blur or `trigger()`, not on every keystroke, so a separate `useState` is required for the debounce effect to react independently. `isDuplicate` itself is not yet consumed by any UI element (button gating is a follow-up task) — it is set for future consumption, which is why `npm run lint` currently reports it as an unused variable.
...
- Updated `SensesTable.tsx`: added "Sense" column between "Word" and "Translation" rendering `sense.meaning_summary || '—'` with `max-w-xs truncate` and `title` attribute for overflow.
